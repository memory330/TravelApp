...
import com.wxmylife.betty.base.BuildConfig;
import com.wxmylife.betty.base.R;
...
public class SplashView extends View {
    ...
    public static final int DEFAULT_ROTATION_RADIUS = 90;
        // do not leave this dimension as default since it is in px
    ...
    private boolean mRemoveFromParentOnEnd = true;
        // a flag for removing the view from its parent once the animation is over
    ...
    private float mCircleRadius = DEFAULT_CIRCLE_RADIUS;
        // the radius of each individual small circle
    ...
    private long mRotationDuration = DEFAULT_ROTATION_DURATION;
        // the duration, in ms, for one complete rotation of the circles
    private long mSplashDuration = DEFAULT_SPLASH_DURATION;
        // the duration, in ms, for the splash animation to go away
    ...
    private void setupAttributes(AttributeSet attrs) {
        ...
        for (int i = 0; i < numAttrs; ++i) {
            ...
            if (attr == R.styleable.NewsDigestSplashView_removeFromParentOnEnd) {
                setRemoveFromParentOnEnd(a.getBoolean(i, DEFAULT_REMOVE_FROM_PARENT_ON_END));
            } else if (attr == R.styleable.NewsDigestSplashView_circleRadius) {
                setCircleRadius(a.getDimensionPixelSize(i, DEFAULT_CIRCLE_RADIUS));

            } else if (attr == R.styleable.NewsDigestSplashView_rotationRadius) {
                setRotationRadius(a.getDimensionPixelSize(i, DEFAULT_ROTATION_RADIUS));

            } else if (attr == R.styleable.NewsDigestSplashView_rotationDuration) {
                setRotationDuration(a.getInteger(i, DEFAULT_ROTATION_DURATION));

            } else if (attr == R.styleable.NewsDigestSplashView_splashBackgroundColor) {
                setSplashBackgroundColor(a.getColor(i, DEFAULT_SPLASH_BG_COLOR));

            } else if (attr == R.styleable.NewsDigestSplashView_splashDuration) {
                setSplashDuration(a.getInteger(i, DEFAULT_SPLASH_DURATION));

            } else if (attr == R.styleable.NewsDigestSplashView_circleColors) {
    ...
    /**
     * Initialized the view properties. Not much is done in this method since most variables already
     * have set defaults
     */
    ...
    /**
     * Setter for the background color. Do not use setBackgroundColor otherwise the view will not
     * draw a transparent hole
     */
    ...
    /**
     * Setter for the duration of the splash animation to take place. The animation has 3 parts, so
     * this duration will be divided to 3 for each animation
     */
    ...
    /**
     * Setter for the colors of the rotating circles. If the given integers are not actual colors,
     * no circles will be produced.
     */
    ...
    /**
     * Setter for the flag to remove or keep the view after the animation is over. This is set to
     * true by default. The view must be inside a ViewManager
     * (or ViewParent) for this to work. Otherwise, the view will not be removed and a warning log
     * will be produced.
     */
    ...
    /**
     * Starts the splash animation. If a listener is provided it will notify the listener on
     * animation events
     */
    ...
    /**
     * Override this method to cache some dimensional values, so that they don't have to be
    ...
    /**
     * Called right before first onDraw takes place and sets the initial animation state. mState
     * must be null at this point for this to work.
     * This is necessary for all parameters to be set correctly. Otherwise, for example, if the
     * RotationState is constructed before animation duration is set, the
    ...
    /**
     * Draws the current state. The actual draw state is delegated to State class and draw helper
     * methods.
     */
    @Override
    ...
    /**
     * Checks preferences and removes the view from its parent view if it can. Generates a DEBUG log
    ...
    private void removeFromParentIfNecessary() {
        ...
        if (mRemoveFromParentOnEnd) {
            ...
            if (parent != null && parent instanceof ViewManager) {
                ...
                Log.w(TAG,
                    "splash view not removed after animation ended because no ViewManager parent was found");
            }
        }
    }
    ...
    private class RotationState extends SplashState {
        ...
        /**
         * The animator needs to be canceled on state change, otherwise the rotation angle will keep
         * changing and the view will leak
         * even after it is destroyed. This method takes care of that, but is not a very nice way of
         * doing so since it requires typecasting
    ...
    /**
     * In this state an animator is used to make the circles bounce back a little and then fall into
            ...
            animator.addUpdateListener(new AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animator) {
                    ...
                    if (mSplashListener != null) {
                        mSplashListener.onUpdate(
                            (float) animator.getCurrentPlayTime() / animator.getDuration() / 3);
                    }
                }
            });
    ...
    /**
     * This state is used to make the only circle visible at this time get a little larger then
